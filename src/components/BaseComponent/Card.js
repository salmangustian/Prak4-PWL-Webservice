...
export const Card = (props) => {
  ...
  const fetchProject = async () => {
    const response = await fetch(
      `https://server-salmangustian.herokuapp.com/api/projects/${props.project.id}`
    );
    setProject({
  ...
  const comment = async () => {
    ...
    if (text) {
      const response = await fetch(`https://server-salmangustian.herokuapp.com/api/comments`, {
        method: "POST",
        headers: {
